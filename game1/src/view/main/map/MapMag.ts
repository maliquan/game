class MapMag {
    ...
    public static TYPE_LINE:number = -1;
    public totalMapH:number = 0;//地图总高度
    public oldStartY:number = 0;//之前的起始点
    public mapDataList:ParserMapVo[] = [];//地图数据

    private static _instance:MapMag;
    private curChapterVo:ParserMapVo;//当前关卡数据
    private group:MapGroup;
    private scroller:eui.Scroller;
    private _cacheItemVoList:ItemVo[] = [];//缓存数据数组
    private itemVoList:ItemVo[] = [];//所有的显示物体
    private _depthList:number[] = [];//每个对象都有唯一的层级，保存当前显示的物体的层级

    static get instance():MapMag {
        if (!this._instance) {
    ...
    public init(scene:ViewCtl):void{
        if(!this.scroller){
    ...
    public findStartY():number{
        var startY:number = this.curChapterVo.y - Global.STAGE_H*0.5;
    ...
    private updata(e:egret.Event):void{
        var newStartY:number = this.group.scrollV;
        //防止频繁刷新，2个像素以上移动再刷新
    ...
    /* 要添加的数据列表，当前的起始y坐标 */
    private addRender(renderDataList:ParserMapVo[]):void {
        var len:number = renderDataList.length;
        var curChapterId:number = UserInfo.ins.curChapterId;
        var chapterList:any[] = UserInfo.ins.chapterDataList;
        for (var i:number=0; i<len; i++) {//根据数据，添加到舞台
            var itemVo:ItemVo = this.popItemVo();
            itemVo.vo = renderDataList[i];
            var vo:ParserMapVo = itemVo.vo;
            if (vo.dataId > 0) {
    ...
    /* 根据起始点找到需要添加的物体的数据 */
    private getParserMapVo(startY:number):ParserMapVo[] {
        var voList:ParserMapVo[] = [];
        var list:ParserMapVo[] = this.mapDataList;
        var len:number = list.length;
        var bottomY:number = startY + Global.STAGE_H;
        //找到所有底部出现在屏幕范围内的物体
        for (var i:number = 0; i < len; i++) {
            var vo:ParserMapVo = list[i];
            if (vo.bottom >= startY && vo.y <= bottomY) {
                ...
                if (this._depthList.indexOf(vo.depth) == -1) {
                    ...
                    voList.push(vo);
//                    console.log("add objId: " + vo.dataId);
                }
            }
        }
        return voList;
    ...
    private sortY():void {
        ...
        var len:number = this.itemVoList.length;
        var bottomY:number = this.oldStartY + Global.STAGE_H;
        for (var i:number = 0; i < len; i++) {
...
//            }
            ...
            //排序出来一个，加载一个
            var itemVo:ItemVo = this.itemVoList[i];
            var isRemove:boolean = itemVo.move(this.group, this.oldStartY, bottomY);
            if(isRemove){
    ...
    /* 根据关卡id更新关卡 */
    public updataBtnId(id:number):void{
        var len:number = this.itemVoList.length;
        for (var i:number = 0; i < len; i++) {
    ...
    /* 根据id找到对应的按钮数据 */
    private getItemVoById(id:number):ParserMapVo {
        var voList:ParserMapVo[] = this.mapDataList;
        var len:number = voList.length;
        for (var i:number = 0; i < len; i++) {
    ...
    /* 收到一个ItemVo，存起来 */
    private pushItemVo(itemVo:ItemVo):void {
        this._cacheItemVoList.push(itemVo);
    }

    /* 产出一个ItemVo，如果没有new */
    private popItemVo():ItemVo {
